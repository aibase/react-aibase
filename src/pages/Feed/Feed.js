...
class Feed extends Component {
  ...
  componentDidMount() {
    ...
    this.loadPosts();
    const socket = openSocket('http://localhost:8081');  // open Socket with the Server
  ...
  // Load Posts data from the server including imageUrl
  loadPosts = direction => {
    if (direction) {
  ...
  finishEditHandler = postData => {
    ...
    formData.append('sheets', postData.sheets);
